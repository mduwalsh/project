...
specifies distribution $\bm{\mu}$ according to rule (see \cite{VoseWright1998})
\[
...
Crossover like mutation also provides mechanism for injection of new strings into new generation population. 
...
Abusing notation, $\bm{\chi}$ can be considered as a {\em crossover rate} that specifies the distribution $\bm{\chi}$ given by rule (see \cite{VoseWright1998})
\[
...
$\mathcal{R}$ is a commutative ring formed a set of haploids (i.e. binary strings of length $\ell$). 
If $x \in \mathcal{R}$, then $x = \langle x_0, x_1, .., x_{\ell - 1} \rangle$. Operations ($+$ and $\cdot$) 
acting on elements of $\mathcal{R}$ () in this section are component-wise addition and multiplication modulo 2. 

If $n \in \mathcal{R}$ and $t \in \mathcal{R}$, and $N$ is cardinality of $\mathcal{R}$, 
the Walsh matrix is defined by
\[
W_{n,t} = N^{-1/2} (-1)^{n^T t}
\]
where $N^{-1/2}$ is normalization factor.
...
\]
The practical importance of this symmetry is that the transform and inverse are the same mathematical operation, and \textit{Walsh matrix} is its own inverse,
\[
W = W^{-1}.
\]
...
Computation of the discrete Walsh transform given by equation (\ref{WalshT}) might take $n^2$ operations (addition or subtraction) if implemented naively.
An algorithm using matrix factorization techniques is found to perform the transformation in $n \log_2 n$ operations.
...
We adapt Walsh transform methods which have already been established for Vose's haploid model (see \cite{VoseWright1998}) 
for computing evolutionary trajectories. Adaptation of Walsh transformation efficiently models infinite diploid population 
evolution, making feasible comparisons between finite and infinte diploid population short-term evolutionary behavior.
Evolution equation (\ref{model4}), specialized to Vose's infinite population model without selection is
\[
p_g^\prime \; = \; (\sigma_g \nudge p)^T M \, (\sigma_g \nudge p)
\]
