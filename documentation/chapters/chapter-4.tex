...
\caption{{\bf {Regression parameters:}} multi-plot for generation
...
d \; \approx \; k/\sqrt{N}
\end{equation}
which agrees with the expected rate of convergence for the single-step
haploid case \cite{Vose1999b}; the distance is inversely proportional
to square root of population size. 
\cite{Vose1999b} calculated variance of next generation population with respect to expected population as 
\[
E(\| \bm{q} - \mathcal{G}(\bm{p}) \|^2) = (1 - \|\mathcal{G}(\bm{p})\|^2) / \bm{N}
\] 
...
which is convex function. Then $E(\| \bm{q} - \mathcal{G}(\bm{p}) \|^2)$ becomes $E(\phi (x))$. From Jensen's inequality, %\cite{Wiki1},
...
E(\| \bm{q} - \mathcal{G}(\bm{p}) \|) \leq \sqrt{(1 - \|\mathcal{G}(\bm{p})\|^2) / \bm{N}}
\end{equation}
Equation \ref{covergenceDistance} is in congruence with equation \ref{covergenceRHS}
...
finite populations \cite{CrowKimura}.
