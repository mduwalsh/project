...
finite populations (see \cite{CrowKimura}). And partly because the variance between 
finite and infinite populations predicted by Vose is for single step case, 
and as generation increases difference between actual finite population and infinite population accumulates 
which can be seen in figure \ref{nonlinear}. 
...
Each solid edge represents one generation, each node represents population point, red edge denotes finite population path, 
and blue line denotes infinite population path from each node}
\label{nonlinear}
\end{center}
\end{figure}
