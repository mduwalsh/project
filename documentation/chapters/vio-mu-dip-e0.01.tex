...
will not die out completely since the Markov chain is regular 
and can pick up at some point in the long run to oscillate again.
...
and finite population shows behavior similar to infinite population as finite population size grows. 
...
\end{table}
From table \ref{distanceMuDipEps0.01}, average distance calculated for $N\,=\,4096$ is $0.0156$, 
for $N\,=\,40960$ is $0.0049$ and for $N\,=\,81920$ is $0.0035$. The results in table \ref{distanceMuDipEps0.01} show average distance 
between finite population and limit $\bm{z^\ast}$ approaches expected single step distance 
between finite and infinite populations. The distance decreased as $1/\sqrt{N}$. 
Also, the distance is smaller in diploid than in haploid populations with $\bm{\epsilon}\,=\,0.01$.
