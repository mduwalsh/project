...
void plot_data_all(unsigned long b, unsigned long g, unsigned long n, double epsilon)
{
  ...
  sprintf(ofn, "b%lug%lun%lueps%.2lf_osc_inf_hap.eps", b, g, n, epsilon);
  sprintf(title, "infinite haploid l:%lu, g:%lu, n:%lu, eps:%.2f", b, g, n, epsilon);
  plot(ifn, 3, title, "g", "d", 0, ofn);
  // diploid infinite
  sprintf(ifn, "%s_osc_inf_diploid.dat", str);
  sprintf(ofn, "b%lug%lun%lueps%.2lf_osc_inf_dip.eps", b, g, n, epsilon);
  sprintf(title, "infinite diploid l:%lu, g:%lu, n:%lu, eps:%.2f", b, g, n, epsilon);
  plot(ifn, 3, title, "g", "d", 0, ofn);
  // haploid finite
...
int main()
{
  ...
  for(li = 0; li < ls; li++)
    for(ni = 0; ni < ns; ni++)	
      for(gi = 0; gi < gs; gi++)
        for(ei = 0; ei < es; ei++, i++){
	  plot_data_all(L[li], G[gi], (unsigned long) pow(2, N[ni]), Epsilon[ei]);	           
      }
  printf("\n%lu\n", i);
  return 0;
}
