...
smoother than graph showing distance between finite haploid population and infinite population. The average amplitude of oscillations data are 
...
\caption{\textbf{Average amplitude of oscillation:} $A$ is average amplitude of oscillation, $L$ is genome length $\ell$, and $N$ is population size}
\label{osc_amp}
\end{center}
\end{figure}
...
and since genome length of diploid population is twice in length as of haploid population, 
...
jump to different level and staying there for a while. Figures \ref{fig:osc_12d_N4096} and \ref{fig:osc_14d_N4096} show such tendency for 
...
when high surge is omitted from plot of \ref{fig:osc_14d_N4096}. 
It could be because as length of string increases, 
number of fixed points also increases, and finite populations tend to stay near those fixed points. 
But when population size is large, finite populations intend to follow infinite population, and infinite population tends to 
