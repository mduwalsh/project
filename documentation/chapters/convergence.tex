...
\caption{\textbf{Regression parameters:} multi-plot for generation  $n \in \{1,2,4,8,16,32,64,128\}$.}
...
which is convex function. Then $\mathcal{E}(\| \bm{q} - \mathcal{G}(\bm{p}) \|^2)$ becomes $\mathcal{E}(\phi (x))$. From Jensen's Inequality (see \cite{JensenInequality}),
